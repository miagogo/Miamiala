$(function(){
	...
	var $item = $('#leftBox .commendBox .picBox li');	
			$item.hover(function(){
				...
				$(this).children('div').css({ display:'block', left:'100px'});		 
			},function(){
		...
		var $item2 = $('#bottomBox .gameListBox .kindBox ul li');	
			$item2.hover(function(){
				var $class = $(this).parent().parent().attr('class'),
						...
						if( $class =='kindBox lastOne' ){
							$newLeft ='90px';
						}
				
				$(this).css({ zIndex:800}).stop(true,false).animate({ backgroundColor:'#c5e9ff'},300, 'easeInQuart');
				$(this).children('div').css({ display:'block',  left:$newLeft});	
			},function(){
